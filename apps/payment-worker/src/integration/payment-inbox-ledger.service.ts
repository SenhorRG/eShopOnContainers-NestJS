...
import { PAYMENT_REDIS_CLIENT } from '../infrastructure/redis.module';

@Injectable()
export class PaymentInboxLedgerService {
  private readonly log = new Logger(PaymentInboxLedgerService.name);
  private readonly store: InboxLedgerStore;
  private readonly inboxMeters = createRedisInboxMeters('eshop.payment.inbox.redis');

  constructor(@Inject(PAYMENT_REDIS_CLIENT) redis: Redis) {
    this.store = createRedisInboxStore(adaptIoredisInboxClient(redis), { keyPrefix: 'eshop:inbox:payment:' });
  }
